...
### 修复：下载在图片全部下完后卡死、不合并 PDF（任务空闲看门狗）
- **现象**：图片 0~90%（download 阶段）正常推进，最后一张图下完后任务永远停在
  「运行中」，PDF（90~100）不再开始 —— UI 表现为"卡住"。
- **根因**：单张图片在 CDN 主域连接超时（实测 `curl(28)` 30s 超时、进入重试 `[1/5]`）
  后，jmcomic 下载器该请求真正挂起，`download()` 的线程池 `join` 被阻塞而**永不返回**
  （`@@PROGRESS@@` 停在 download 100%），上游 PDF 合并逻辑（`export_result_pdfs`）
  根本执行不到 —— 下载核心自身的网络挂起，而非 PDF 合并代码问题（离线测试已覆盖
  并通过）。
- **修复**：`JobManager` 新增**空闲看门狗**（`_idle_watchdog`）：对 running 任务做
  「无进展检测」——连续 `IDLE_TIMEOUT`(180s) 无任何 stdout/stderr 输出且已运行超
  `IDLE_MIN_ELAPSED`(60s)，即终止子进程并把任务收敛为明确失败，`summary` 记录
  `timed_out/reason`，避免任务无限挂起不可终止。正常有输出的任务（含长耗时 PDF 合并）
  不会被误杀。
